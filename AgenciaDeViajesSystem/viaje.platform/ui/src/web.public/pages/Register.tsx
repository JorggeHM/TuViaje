/**
 * Register.tsx — Página de registro de nuevo usuario (ruta: /register)
 *
 * Diseño espejado al Login.tsx: panel de formulario a la izquierda
 * y panel naranja con beneficios a la derecha.
 *
 * El formulario actualmente solo tiene estructura visual (e.preventDefault()).
 * Para conectarlo al backend, usar AuthService.register(email, password, name)
 * definido en infrastructure/services/auth.service.ts.
 *
 * Conexión con el backend:
 *   POST /auth/register → crea la cuenta y devuelve los datos del usuario
 *   Después del registro, redirigir a /login para que el usuario se autentique
 *
 * Navegación relacionada:
 *   "Ya tengo una cuenta" → /login
 */
import { Mail, User, Plane } from "lucide-react";
          ...
          <div className="mt-8 space-y-3 text-left w-full max-w-xs">
            {[
              ...
              <div key={item} className="flex items-center gap-2.5">
                <span className="flex-shrink-0 flex items-center justify-center w-5 h-5 rounded-full bg-white/25 text-white text-xs font-bold">
                  ✓
                </span>
